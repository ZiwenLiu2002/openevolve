...
export OE_STAGE2_SAVE_DIR="gpt_stage2_traces_0420_3"
...
# model loading: base model + overlay
export OPENEVOLVE_MODEL_PATH="${OPENEVOLVE_MODEL_PATH:-openai/gpt-oss-20b}"
export OPENEVOLVE_OVERLAY_PATH="${OPENEVOLVE_OVERLAY_PATH:-../../../gsm8k/overlays/gpt-oss-20b-wanda-s050.pt}"

if [ $# -lt 1 ]; then
    echo "Usage: $0 <prompt_file> [additional_args...]"
    echo "Example: $0 gsm8k_prompt.txt --iterations 50"
    exit 1
fi
...
echo "[info] model path (base): $OPENEVOLVE_MODEL_PATH"
echo "[info] overlay path: $OPENEVOLVE_OVERLAY_PATH"
...
python ../../openevolve-run.py \
    "$PROMPT_FILE" \
    evaluator_with_overlay.py \
    --config config.yaml \
    --output gpt_output_0420_3 \
    "$@"